...
exports.run = async (client, message, args) => {
    ...
    if(message.channel.id !== "854878064148217906") return message.reply('bu komut sadece destek kanalında çalışır.').then(message => { message.delete({ timeout: 5000 })})
   if (message.guild.channels.cache.get(db.fetch(`destek_${message.author.id}`))) return false || message.channel.send('Şu anda mevcut bir talebin var.').then(message => { message.delete({ timeout: 5000 })})
    ...
    let yetkili = message.guild.roles.cache.find(x => x.id == "685247231385010307") // YETKİLİ ROL İD
    ...
    message.guild.channels.create(`destek-${message.author.username}`, "text").then(async c => {
        ...
        const category = message.guild.channels.cache.get('854877255523893278') // Kategori id
